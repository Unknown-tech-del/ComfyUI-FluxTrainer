...
#
# --- Local modifications for ComfyUI-FluxTrainer ---
# Vendored from diffusers' transformer_z_image.py (relative imports below made absolute
# so it can live outside the diffusers package) so the two ops below can be made fp8-safe,
# matching Kohya's --fp8_base flag: blanket-casting the whole model to torch.float8_e4m3fn
# (train_network.py's `unet.to(dtype=unet_weight_dtype)`) works fine for ops that go through
# nn.Linear/nn.Conv (autocast promotes fp8<->bf16 automatically for those), but NOT for raw
# elementwise ops touching a Parameter directly -- confirmed empirically:
#   torch.where(mask, fp8_param, bf16_tensor)   -> "Promotion for Float8 Types is not supported"
#   bf16_tensor * fp8_param                     -> same error
# Two spots in this file hit that: RMSNorm's `hidden_states * self.weight` (patched below via
# a module-level monkeypatch of diffusers.models.normalization.RMSNorm.forward, since Attention's
# internal qk_norm also instantiates that same class) and `_prepare_sequence`'s
# `torch.where(mask, pad_token, feats_cat)` (patched at the call site, search "fp8-safe").
...
def _fp8_safe_rmsnorm_forward(self, hidden_states):
    """Replaces RMSNorm.forward so it tolerates a float8-cast self.weight (fp8_base training).
    Functionally identical to upstream for bf16/fp16/fp32 weights: computes the normalization in
    float32, then always casts `weight` to match the (post-norm) tensor's dtype right before the
    elementwise multiply -- upstream only did that cast when weight.dtype was fp16/bf16, which
    left fp8 (and fp32) weights to hit PyTorch's "no implicit promotion for float8" error.
    """
    ...
    if self.weight is not None:
        hidden_states = hidden_states.to(input_dtype) * self.weight.to(input_dtype)
...
if getattr(RMSNorm.forward, "_zimage_fp8_patched", False) is False:
    _fp8_safe_rmsnorm_forward._zimage_fp8_patched = True
    RMSNorm.forward = _fp8_safe_rmsnorm_forward
...
class ZImageTransformer2DModel(ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin):
    ...
    def _prepare_sequence(
        ...
        # fp8-safe: torch.where can't implicitly promote a float8 pad_token against a bf16
        # tensor (unlike nn.Linear/nn.Conv, which autocast handles), so match dtypes explicitly.
        feats_cat = torch.where(mask, pad_token.to(feats_cat.dtype), feats_cat)
